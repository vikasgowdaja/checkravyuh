'use client';

import Link from 'next/link';
import { useParams } from 'next/navigation';
import { useEffect, useState } from 'react';

import { GuidedTrainer } from '../../../components/guided-trainer';
import { fetchJson, type TrapDetail, type TrapTrainingResponse } from '../../../lib/api';

export default function PracticePage() {
  const params = useParams<{ id: string }>();
  const trapId = params.id;
  const [trap, setTrap] = useState<TrapDetail | null>(null);
  const [training, setTraining] = useState<TrapTrainingResponse | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let isMounted = true;

    async function loadPractice() {
      try {
        const [trapData, trainingData] = await Promise.all([
          fetchJson<TrapDetail>(`/traps/${trapId}`),
          fetchJson<TrapTrainingResponse>(`/traps/${trapId}/training`),
        ]);

        if (!isMounted) {
          return;
        }

        setTrap(trapData);
        setTraining(trainingData);
        setError(null);
      } catch (loadError) {
        if (!isMounted) {
          return;
        }

        setError(loadError instanceof Error ? loadError.message : 'Unable to load practice.');
      }
    }

    if (trapId) {
      void loadPractice();
    }

    return () => {
      isMounted = false;
    };
  }, [trapId]);

  return (
    <main className="page-container">
      {trap && training ? (
        <>
          <section className="panel-card">
            <div className="eyebrow">Practice</div>
            <h1 style={{ fontSize: '3rem', margin: '12px 0 12px' }}>{trap.title}</h1>
            <p className="muted-copy" style={{ lineHeight: 1.7, maxWidth: 820 }}>
              White moves are predetermined. Black is yours. The backend validates every response so
              you can repeat the trap until the pattern becomes automatic.
            </p>
            <div className="action-row" style={{ marginTop: 20 }}>
              <Link href={`/watch/${trap.id}`} className="action-button secondary">
                Watch first
              </Link>
              <Link href={`/tour/${trap.id}`} className="action-button secondary">
                Guided tour
              </Link>
              <Link href={`/analysis/${trap.id}`} className="action-button secondary">
                Analysis board
              </Link>
              <Link href={`/traps/${trap.id}`} className="action-button secondary">
                Back to trap detail
              </Link>
              <Link href="/review" className="action-button secondary">
                Go to review queue
              </Link>
            </div>
          </section>

          <section style={{ marginTop: 24 }}>
            <GuidedTrainer trapId={trap.id} mode="practice" turns={training.practice.turns} />
          </section>
        </>
      ) : error ? (
        <section className="panel-card" style={{ background: '#f8e1db', color: '#702d22' }}>
          {error}
        </section>
      ) : (
        <section className="panel-card">Loading practice...</section>
      )}
    </main>
  );
}